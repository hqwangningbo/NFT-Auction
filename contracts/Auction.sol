...
contract Auction  {
    ...
    //发布商品
    function publishProduct(uint256 _lowestPrice,uint256 _nftTokenId,string _desc) public{
        require(msg.sender == tokenAddress.ownerOf(_nftTokenId));
         prodcutId.increment();
    ...
    function acutionEnd(uint256 _productId) payable public{
        ...
        if(product.buyer != address(0)){
         product.seller.transfer(product.highestPrice);
        //  tokenAddress.approve(product.buyer,product.nftTokenId);
        //  tokenAddress.transferFrom(product.seller,product.buyer,product.nftTokenId);
        }
        product.status = 1;
        //触发结束事件
